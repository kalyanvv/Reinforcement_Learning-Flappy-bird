...
def train(opt):
    ...
    while iter < opt.num_iters:
        ...
        y_batch = torch.cat(
            tuple(reward if terminal else reward + opt.gamma * torch.max(prediction).item() for reward, terminal, prediction in
                  zip(reward_batch, terminal_batch, next_prediction_batch)))

        q_value = torch.sum(current_prediction_batch * action_batch, dim=1)
        optimizer.zero_grad()
        loss = criterion(q_value, y_batch.detach())  # Detach the tensor here
        loss.backward()
        optimizer.step()
        ...
        print("Iteration: {}/{}, Action: {}, Loss: {}, Epsilon {}, Reward: {}, Q-value: {}".format(
            ...
            action,
            loss,
            epsilon, reward, torch.max(prediction).item()))
